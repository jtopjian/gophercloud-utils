...
func GetManifest(client *gophercloud.ServiceClient, opts GetManifestOpts) ([]Manifest, error) {
	...
	// TODO: test this
	if opts.Headers.ObjectManifest != "" {
		v := strings.SplitN(opts.Headers.ObjectManifest, "/", 1)
		if len(v) != 2 {
			return nil, fmt.Errorf("Unable to parse object manifest")
		}
	...
	if opts.Headers.StaticLargeObject {
		if opts.Manifest == "" {
	...
	m := Manifest{
		Hash:  opts.Headers.ETag,
		Bytes: opts.Headers.ContentLength,
	}
